...
class SECParser:
    ...
    def generate_filing_report(self, symbol: str) -> str:
        ...
        latest_10k = self.get_latest_10k(symbol)
        if latest_10k:
            report.append("## Latest 10-K (Annual Report)")
        ...
        latest_10q = self.get_latest_10q(symbol)
        if latest_10q:
            report.append("## Latest 10-Q (Quarterly Report)")
        ...
        if not recent_8k.empty:
            report.append("## Recent 8-K Filings (Last 90 Days)")
            for _, row in recent_8k.head(5).iterrows():
                report.append(
                    f"- {row['filing_date'].strftime('%Y-%m-%d')}: {row['primary_doc_description']}"
                )
            report.append("")
